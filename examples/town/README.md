...
Set production `PUBLIC_URL` and `PLC_DIRECTORY` in the Worker's **Settings → Variables and Secrets**, and its custom domain in **Settings → Domains & Routes**. Do not set the development resolver in production. Deployments preserve these settings. Type generation reads `.dev.vars.example`, not private local values. See the [deployment guide](../../docs/DEPLOYMENT.md).
