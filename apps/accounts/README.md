...
Accounts accepts `atproto`, valid `repo` permissions for any collection, and valid `blob` upload permissions such as `blob:image/*`. `@atproto/oauth-scopes` validates permission semantics; each requested scope string must also appear in client metadata. Consent and tokens retain exactly the requested scopes, and refresh never expands them. Authorization-server metadata lists static scopes only, not the unbounded set of collection and MIME permissions. Upload permissions do not authorize profile or other record writes; those need separate repository grants.

The consent UI renders actions, collections, and allowed upload MIME patterns. Its optional collection-name map supplies friendly labels such as “Bluesky posts” and “Bluesky profile”; it does not authorize a collection or load its schema. Unknown collections remain usable and display their NSIDs. Permission sets (`include:`), RPC, identity, and account permissions remain unsupported. Supporting permission sets requires authenticated Lexicon resolution, namespace checks, and matching resource-server enforcement, not just a display label.
