...
The package exposes `atprotoOAuthProvider(options)`. Application callbacks resolve which DID subjects an authenticated Better Auth user can authorize, revalidate a selected subject, render the authorization page, create the login redirect, and issue resource-server access tokens. The package does not import application code or assume how users and DIDs are related. Each consumer owns its authorization policy and storage behind those callbacks.
...
- a selected DID in the OAuth session and top-level token-response `sub`.

PAR requests expire after five minutes, authorization codes expire after one minute, and public-client sessions have a fixed maximum age of two weeks. Access tokens expire after five minutes. The application token callback must bind the selected DID as `sub`, the resource origin as `aud`, and the DPoP JWK thumbprint as `cnf.jkt`.
...
Confidential `private_key_jwt` clients are intentionally rejected and are not advertised. Dynamic registration, client secrets, OIDC, client credentials, and implicit grants are not supported or advertised.
